...
import { Search, Star, MapPin, Clock, ChevronRight, TrendingUp } from "lucide-react";
      ...
      <section className="relative bg-slate-900 rounded-3xl overflow-hidden px-8 py-12 md:px-14 md:py-16">
        ...
        <div className="relative max-w-2xl">
          ...
          <p className="text-slate-400 text-base md:text-lg mb-8 max-w-lg">
            Peluquerias, constructoras, tintorerías, clinicas y más — agenda en segundos con negocios verificados.
          </p>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {featured.map((biz) => (
            ...
            >
              {/* Banner de color */}
              <div className={`h-24 bg-gradient-to-br ${biz.gradient} relative flex items-end p-3`}>
                <div className="w-10 h-10 bg-white rounded-xl shadow-md flex items-center justify-center text-lg font-bold text-slate-700">
                  {biz.name.charAt(0)}
                </div>
                {biz.openNow && (
                  <span className="absolute top-3 right-3 text-xs font-semibold bg-emerald-500 text-white px-2 py-0.5 rounded-full">
                    Abierto
                  </span>
                )}
              </div>
              <div className="p-4">
                <p className="text-sm font-bold text-slate-900 group-hover:text-indigo-700 transition-colors leading-tight">{biz.name}</p>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex items-center gap-1 text-slate-400">
                    ...
                    <span className="text-xs truncate max-w-[80px]">{biz.city}</span>
                  </div>
                </div>
              </div>
            </Link>
      ...
      <section>
        ...
        <div className="space-y-3">
          {recent.map((biz) => (
            ...
            >
              <div className={`w-14 h-14 bg-gradient-to-br ${biz.gradient} rounded-xl flex items-center justify-center text-white text-xl font-bold flex-shrink-0`}>
                {biz.name.charAt(0)}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <div className="flex items-center gap-1 text-slate-400">
                    ...
                    <span className="text-xs">{biz.hours}</span>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-end gap-1 flex-shrink-0">
                {biz.openNow
                  ? <span className="text-xs font-medium text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">Abierto</span>
                  : <span className="text-xs font-medium text-slate-400 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-full">Cerrado</span>
                }
